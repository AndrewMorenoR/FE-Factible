...
namespace Model.Query
{
    public class DbQuery : IDbQuery
    {
        ...
        public DataTable GetDocumentsData()
        {
            try
            {
                ...
                //               //"CAST(TO_VARCHAR(CAST('20220601' AS DATE)) AS NVARCHAR(20)) \"fechadocumento\",  " +
                //               //"CAST(TO_VARCHAR(CAST('20220605' AS DATE)) AS NVARCHAR(20)) \"fechavencimiento\", " +
                ...
                //                               "'0' \"correocopia\", " +
                //                               "CASE WHEN UPPER(\"PymntGroup\") LIKE '%CONTADO%' THEN '1' ELSE '2' END \"FP_idmetodopago\", " +
                //                               "'0' \"FP_idmediopago\",  " +
                ...
                //                               "(SELECT T9.\"County\" FROM \"" + this.SCHEMA + "\".CRD1 T9 WHERE T9.\"CardCode\" =L0.\"CardCode\" AND T9.\"U_SCL_DirMM\" = 'S') \"ADQUIR_idciudad\", " +
                //                               //"(SELECT T9.\"County\" FROM \"" + this.SCHEMA + "\".CRD1 T9 WHERE T9.\"CardCode\" = L0.\"CardCode\" AND T9.\"LineNum\" = (SELECT MIN(T10.\"LineNum\") FROM \"" + this.SCHEMA + "\".CRD1 T10 WHERE T10.\"CardCode\" = T9.\"CardCode\")) \"ADQUIR_idciudad\", " +
                ...
                //                                "CAST(CAST(L11.\"TOTALBRUTO\"  AS NUMERIC(19,2)) AS NVARCHAR(23)) \"TOTAL_totalbruto\", " +
                //                "CAST(CAST(IFNULL(L10.\"TOTAL_BASEIMP\",0) AS NUMERIC(19,2)) AS NVARCHAR(23)) \"TOTAL_baseimponible\", " +
                //                "CAST(CAST(L11.\"TOTALBRUTO\"+L7.\"TOTAL_IMPUESTO\" - IFNULL(L0.\"DiscSum\",0) AS NUMERIC(19,2)) AS NVARCHAR(23)) \"TOTAL_totalbrutoconimpuestos\", " +
                ...
                //                "cast(CASE when CAST(CASE WHEN L0.\"DocCur\" IN('COP','$') THEN L0.\"DiscSum\" ELSE L0.\"DiscSumFC\" END AS NUMERIC(19,2))<>0 then CAST(CASE WHEN L0.\"DocCur\" IN('COP','$') THEN abs(L0.\"DiscSum\") ELSE abs(L0.\"DiscSumFC\") END AS NUMERIC(19,2)) else CAST(IFNULL(L12.\"LineTotal\",0) AS NUMERIC (19,2)) end AS NVARCHAR(23)) \"TOTAL_totalcargos\", " +
                //                "CAST(CAST('0' AS NUMERIC (19,2))AS NVARCHAR(23)) \"TOTAL_totalanticipos\", " +
                //                "CAST(CAST((L11.\"TOTALBRUTO\"+L7.\"TOTAL_IMPUESTO\" + IFNULL(L12.\"LineTotal\",0) - IFNULL(L0.\"DiscSum\",0)) AS NUMERIC(19,2)) AS NVARCHAR(23)) \"TOTAL_totalapagar\", " +
                ...
                //                "SUM(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineVat\" ELSE \"LineVatlF\" END) \"TOTAL_IMPUESTO\"" +
                //                "FROM \"" + this.SCHEMA + "\".INV1 GROUP BY \"DocEntry\") L7 ON L0.\"DocEntry\"=L7.\"DocEntry\" " +
                //                "LEFT JOIN \"" + this.SCHEMA + "\".OCRN L8 ON L0.\"DocCur\"=L8.\"CurrCode\" LEFT JOIN \"" + this.SCHEMA + "\".OADM L9 ON 1=1 " +
                //                "LEFT JOIN(SELECT T0.\"DocEntry\",SUM((CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END )-(CAST(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END AS NUMERIC(19,2))* CASE WHEN T0.\"DiscPrcnt\" <> 0 THEN T0.\"DiscPrcnt\" ELSE IFNULL(T0.\"DiscPrcnt\",0) END /100)) \"TOTAL_BASEIMP\" FROM \"" + this.SCHEMA + "\".INV1 T1 JOIN \"" + this.SCHEMA + "\".OINV T0 ON T0.\"DocEntry\" = T1.\"DocEntry\" WHERE \"VatPrcnt\" <> 0 " +
                //                "GROUP BY T0.\"DocEntry\") L10 ON L0.\"DocEntry\"= L10.\"DocEntry\" "+
                //                "LEFT JOIN(SELECT T0.\"DocEntry\",SUM((CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END )-(CAST(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END AS NUMERIC(19,2))* CASE WHEN T0.\"DiscPrcnt\" <> 0 THEN T0.\"DiscPrcnt\" ELSE IFNULL(T0.\"DiscPrcnt\",0) END /100)) \"TOTALBRUTO\"" +
                //                "FROM \"" + this.SCHEMA + "\".INV1 T1 JOIN \"" + this.SCHEMA + "\".OINV T0 ON T0.\"DocEntry\" = T1.\"DocEntry\" " +
                //                "GROUP BY T0.\"DocEntry\") L11 ON L11.\"DocEntry\"= L0.\"DocEntry\" " +

                //                "LEFT JOIN(SELECT T0.\"DocEntry\", T1.\"LineTotal\" FROM \"" + this.SCHEMA + "\".OINV T0 JOIN \"" + this.SCHEMA + "\".INV3 T1 ON T0.\"DocEntry\" = T1.\"DocEntry\"  JOIN \"" + this.SCHEMA + "\".OEXD T2 ON T1.\"ExpnsCode\" = T2.\"ExpnsCode\" WHERE T2.\"ExpnsName\" LIKE '%CONSUMO%')  L12 ON L12.\"DocEntry\"= L0.\"DocEntry\" " +
                //                "WHERE L0.\"DocSubType\"='--'  " +
                //                "AND L0.\"DocDate\">='20220608' " +
                ...
                //                "LEFT JOIN (SELECT T0.\"DocEntry\",T1.\"DocDate\" FROM \"" + this.SCHEMA + "\".RIN1 T0 JOIN \"" + this.SCHEMA + "\".ODLN T1 ON T0.\"BaseType\"=T1.\"ObjType\" AND T0.\"BaseEntry\"=T1.\"DocEntry\" WHERE T0.\"VisOrder\"=0) L4 ON L4.\"DocEntry\"=L0.\"DocEntry\"" +
                //                "LEFT JOIN \"" + this.SCHEMA + "\".OCRD L6 ON L0.\"CardCode\"=L6.\"CardCode\"" +
                //                "LEFT JOIN (SELECT \"DocEntry\",SUM(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END) \"SUBTOTAL\",  " +
                //                "SUM(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineVat\" ELSE \"LineVatlF\" END) \"TOTAL_IMPUESTO\"" +
                //                "FROM \"" + this.SCHEMA + "\".RIN1 GROUP BY \"DocEntry\") L7 ON L0.\"DocEntry\"=L7.\"DocEntry\" " +
                //                "LEFT JOIN \"" + this.SCHEMA + "\".OCRN L8 ON L0.\"DocCur\"=L8.\"CurrCode\" LEFT JOIN \"" + this.SCHEMA + "\".OADM L9 ON 1=1 " +
                //                "LEFT JOIN(SELECT T0.\"DocEntry\",SUM((CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END )-(CAST(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END AS NUMERIC(19,2))* CASE WHEN T0.\"DiscPrcnt\" <> 0 THEN T0.\"DiscPrcnt\" ELSE IFNULL(T0.\"DiscPrcnt\",0) END /100)) \"TOTAL_BASEIMP\" FROM \"" + this.SCHEMA + "\".RIN1 T1 JOIN \"" + this.SCHEMA + "\".ORIN T0 ON T0.\"DocEntry\" = T1.\"DocEntry\" WHERE \"VatPrcnt\" <> 0 " +
                //                "GROUP BY T0.\"DocEntry\") L10 ON L0.\"DocEntry\"= L10.\"DocEntry\" " +
                //                "LEFT JOIN(SELECT T0.\"DocEntry\",SUM((CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END )-(CAST(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END AS NUMERIC(19,2))* CASE WHEN T0.\"DiscPrcnt\" <> 0 THEN T0.\"DiscPrcnt\" ELSE IFNULL(T0.\"DiscPrcnt\",0) END /100)) \"TOTALBRUTO\"" +
                //                "FROM \"" + this.SCHEMA + "\".RIN1 T1 JOIN \"" + this.SCHEMA + "\".ORIN T0 ON T0.\"DocEntry\" = T1.\"DocEntry\" " +
                //                "GROUP BY T0.\"DocEntry\") L11 ON L11.\"DocEntry\"= L0.\"DocEntry\" " +
                //                "LEFT JOIN(SELECT T0.\"DocEntry\", T1.\"LineTotal\" FROM \"" + this.SCHEMA + "\".ORIN T0 JOIN \"" + this.SCHEMA + "\".RIN3 T1 ON T0.\"DocEntry\" = T1.\"DocEntry\"  JOIN \"" + this.SCHEMA + "\".OEXD T2 ON T1.\"ExpnsCode\" = T2.\"ExpnsCode\" WHERE T2.\"ExpnsName\" LIKE '%CONSUMO%')  L12 ON L12.\"DocEntry\"= L0.\"DocEntry\" " +
                //                  "WHERE L0.\"DocDate\">='20220608' " +
                ...
                //                               "'0' \"correocopia\", " +
                //                               "CASE WHEN UPPER(\"PymntGroup\") LIKE '%CONTADO%' THEN '1' ELSE '2' END \"FP_idmetodopago\", " +
                //                               "L0.\"U_SCL_FE_MedioPago\"  \"FP_idmediopago\",  " +
                //                               "CAST(TO_VARCHAR(L0.\"DocDueDate\", 'YYYY-MM-DD hh:mi:ss') AS NVARCHAR(20)) \"FP_fechavencimiento\", " +
                //                               //"CAST(TO_VARCHAR(CAST('20210921' AS DATE)) AS NVARCHAR(20)) \"FP_fechavencimiento\", " +
                //                               "'0' \"FP_identificador\", " +
                //                               "L3.\"ExtraDays\" \"FP_dias\", " +
                ...
                //                "FROM \"" + this.SCHEMA + "\".OPCH L0 JOIN \"" + this.SCHEMA + "\".NNM1 L1 ON L0.\"Series\"=L1.\"Series\" JOIN \"" + this.SCHEMA + "\".OADM L2 ON 1=1 LEFT JOIN \"" + this.SCHEMA + "\".OCTG L3 ON L0.\"GroupNum\" = L3.\"GroupNum\" " +
                //                "LEFT JOIN (SELECT T0.\"DocEntry\",T1.\"DocDate\" FROM \"" + this.SCHEMA + "\".PCH1 T0 JOIN \"" + this.SCHEMA + "\".ODLN T1 ON T0.\"BaseType\"=T1.\"ObjType\" AND T0.\"BaseEntry\"=T1.\"DocEntry\" WHERE T0.\"VisOrder\"=0) L4 ON L4.\"DocEntry\"=L0.\"DocEntry\"" +
                //                "LEFT JOIN \"" + this.SCHEMA + "\".OCRD L6 ON L0.\"CardCode\"=L6.\"CardCode\"" +
                //                "LEFT JOIN (SELECT \"DocEntry\",SUM(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END) \"SUBTOTAL\",  " +
                //                "SUM(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineVat\" ELSE \"LineVatlF\" END) \"TOTAL_IMPUESTO\"" +
                //                "FROM \"" + this.SCHEMA + "\".PCH1 GROUP BY \"DocEntry\") L7 ON L0.\"DocEntry\"=L7.\"DocEntry\" " +
                //                "LEFT JOIN \"" + this.SCHEMA + "\".OCRN L8 ON L0.\"DocCur\"=L8.\"CurrCode\" LEFT JOIN \"" + this.SCHEMA + "\".OADM L9 ON 1=1 " +
                //                "LEFT JOIN(SELECT T0.\"DocEntry\",SUM((CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END )-(CAST(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END AS NUMERIC(19,2))* CASE WHEN T0.\"DiscPrcnt\" <> 0 THEN T0.\"DiscPrcnt\" ELSE IFNULL(T0.\"DiscPrcnt\",0) END /100)) \"TOTAL_BASEIMP\" FROM \"" + this.SCHEMA + "\".PCH1 T1 JOIN \"" + this.SCHEMA + "\".OPCH T0 ON T0.\"DocEntry\" = T1.\"DocEntry\" WHERE \"VatPrcnt\" <> 0 " +
                //                "GROUP BY T0.\"DocEntry\") L10 ON L0.\"DocEntry\"= L10.\"DocEntry\" " +
                //                "LEFT JOIN(SELECT T0.\"DocEntry\",SUM((CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END )-(CAST(CASE WHEN \"Currency\" IN('COP','$') THEN \"LineTotal\" ELSE \"TotalFrgn\" END AS NUMERIC(19,2))* CASE WHEN T0.\"DiscPrcnt\" <> 0 THEN T0.\"DiscPrcnt\" ELSE IFNULL(T0.\"DiscPrcnt\",0) END /100)) \"TOTALBRUTO\"" +
                //                "FROM \"" + this.SCHEMA + "\".PCH1 T1 JOIN \"" + this.SCHEMA + "\".OPCH T0 ON T0.\"DocEntry\" = T1.\"DocEntry\" " +
                //                "GROUP BY T0.\"DocEntry\") L11 ON L11.\"DocEntry\"= L0.\"DocEntry\" " +

                //                "LEFT JOIN(SELECT T0.\"DocEntry\", T1.\"LineTotal\" FROM \"" + this.SCHEMA + "\".OPCH T0 JOIN \"" + this.SCHEMA + "\".PCH3 T1 ON T0.\"DocEntry\" = T1.\"DocEntry\"  JOIN \"" + this.SCHEMA + "\".OEXD T2 ON T1.\"ExpnsCode\" = T2.\"ExpnsCode\" WHERE T2.\"ExpnsName\" LIKE '%CONSUMO%')  L12 ON L12.\"DocEntry\"= L0.\"DocEntry\" " +
                //                "WHERE L0.\"DocSubType\"='--'  " +
                //                "AND L0.\"DocDate\">='20220731' " +
                //                ")LL " +
                //                "WHERE \"DOCNUM\" NOT IN(SELECT \"Code\" FROM \"" + this.SCHEMA + "\".\"@FE_INFO\") " +
                //                "AND \"DOCNUM\" NOT IN(SELECT \"Code\" FROM \"" + this.SCHEMA + "\".\"@SCL_FE_ESTADO\" WHERE \"U_SCL_EstadoDoc\" <> 'M') ORDER BY 1";
 ...
 //               connection.Close();
  ...
  //              connection.Close();
        ...
        public DataTable GetAdjuntosData()
        {
            try
            {
                string queryString = "select \"DOCNUM\", \"ANEXOS_codigo\", \"ANEXOS_ruta\", \"ANEXOS_nombre\" from " + SCHEMA + ".\"ANEXOS\"";
